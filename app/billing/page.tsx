                    ...
                    <div className="max-w-6xl mx-auto">
                        <header className="mb-10">
                            <h1 className="text-4xl font-black text-gray-900 dark:text-white tracking-tight">
                                {isParent ? "Riwayat Pembayaran SPP Anak" : "Billing & Payments"}
                            </h1>
                            <p className="text-gray-500 dark:text-gray-400 font-medium mt-1">
                                {isParent ? "Pantau semua riwayat pembayaran SPP anak Anda, termasuk pembayaran offline di sekolah." : "Manage your school fees and payment history."}
                            </p>
                        </header>

                        {isParent ? (
                            /* PARENT SPECIFIC VIEW: LEDGER STYLE */
                            <div className="space-y-8 animate-fade-in">
                                {/* Parent Stats Overview */}
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm transition-transform hover:scale-[1.02]">
                                        <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Total Terbayar (2026)</p>
                                        <h3 className="text-3xl font-black text-emerald-600 dark:text-emerald-400">{formatCurrency(1500000)}</h3>
                                    </div>
                                    <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm transition-transform hover:scale-[1.02]">
                                        <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Sisa Tagihan</p>
                                        <h3 className="text-3xl font-black text-red-600 dark:text-red-400">{formatCurrency(500000)}</h3>
                                    </div>
                                    <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm transition-transform hover:scale-[1.02]">
                                        <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Status Keuangan</p>
                                        <div className="flex items-center gap-2 mt-1">
                                            <span className="w-3 h-3 bg-green-500 rounded-full animate-pulse"></span>
                                            <h3 className="text-xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">LANCAR</h3>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white dark:bg-gray-800 rounded-[2.5rem] shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                                    <div className="p-8 border-b border-gray-50 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-700/30 flex justify-between items-center">
                                        <h3 className="font-black text-gray-900 dark:text-white uppercase tracking-[0.2em] text-sm">Buku Ledger Pembayaran Siswa</h3>
                                        <div className="flex gap-8">
                                            <button className="text-[11px] font-black text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400 pb-2 uppercase tracking-[0.15em] transition-all">Tahun 2026</button>
                                            <button className="text-[11px] font-black text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-all uppercase tracking-[0.15em] pb-2">Tahun 2025</button>
                                        </div>
                                    </div>
                                        ...
                                        <table className="w-full text-left">
                                            <thead>
                                                <tr className="bg-gray-50 dark:bg-gray-800 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                                                    <th className="px-8 py-5">Periode / Bulan</th>
                                                    <th className="px-8 py-5">Tanggal Bayar</th>
                                                    <th className="px-8 py-5">Metode</th>
                                                    <th className="px-8 py-5">Jumlah</th>
                                                    <th className="px-8 py-5">Verifikasi Sekolah</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-gray-50 dark:divide-gray-700">
                                                {[
                                                    { month: "Januari 2026", date: "05 Jan 2026", method: "Tunai (Kantor)", amount: 500000, type: "offline" },
                                                    { month: "Desember 2025", date: "10 Des 2025", method: "Transfer Bank", amount: 500000, type: "online" },
                                                    { month: "November 2025", date: "02 Nov 2025", method: "Tunai (Kantor)", amount: 500000, type: "offline" },
                                                    { month: "Oktober 2025", date: "08 Okt 2025", method: "Tunai (Kantor)", amount: 500000, type: "offline" },
                                                ].map((row, i) => (
                                                    <tr key={i} className="hover:bg-blue-50/20 dark:hover:bg-blue-900/10 transition-colors group">
                                                        <td className="px-8 py-6 font-black text-gray-900 dark:text-white">{row.month}</td>
                                                        <td className="px-8 py-6 text-sm font-medium text-gray-500 dark:text-gray-400">{row.date}</td>
                                                        <td className="px-8 py-6">
                                                            <div className="flex items-center gap-3">
                                                                <span className={`w-2.5 h-2.5 rounded-full ${row.type === 'offline' ? 'bg-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.4)]' : 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.4)]'}`}></span>
                                                                <span className="text-[10px] font-black text-gray-700 dark:text-gray-300 uppercase tracking-widest">{row.method}</span>
                                                            </div>
                                                        </td>
                                                        <td className="px-8 py-6 font-black text-gray-900 dark:text-white tracking-tighter text-lg">{formatCurrency(row.amount)}</td>
                                                        <td className="px-8 py-6">
                                                            <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400 bg-emerald-50/50 dark:bg-emerald-900/20 px-4 py-2 rounded-xl w-fit border border-emerald-100 dark:border-emerald-800/50">
                                                                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" /></svg>
                                                                <span className="text-[9px] font-black uppercase tracking-[0.2em]">Tervalidasi</span>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                ))}
                                    ...
                                    <div className="p-8 bg-gray-50/50 dark:bg-gray-800/30 border-t border-gray-50 dark:border-gray-700 text-center">
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-loose">Menampilkan data pembayaran resmi tahun ajaran 2025/2026</p>
                                    </div>
                                </div>
                            </div>
                        ) : (
                            /* STUDENT VIEW: TRANSACTION CARDS (EXISTING) */
